...
from app.category_catalog import NO_WEIGH_CATEGORIES
...
def _weight_display(name):
    """Подпись весовой категории для справочника."""
...
def build_categories_catalog_pdf():
    """PDF-справочник всех категорий: дисциплины, ката, веса кумитэ, возраст, команды, разряды."""
    ...
    note_style = _paragraph_style(
        "CatalogNote", styles["Normal"],
        fontSize=9, textColor=colors.HexColor("#444444"), spaceAfter=8,
    )
    story = [
        Paragraph("Справочник категорий", styles["Title"]),
        Paragraph(SPORT_NAME, styles["Center"]),
        Paragraph(f"Код вида спорта: {SPORT_CODE}", styles["Center"]),
        Spacer(1, 0.4 * cm),
        Paragraph(
            "Полный перечень дисциплин, видов программы, весовых и возрастных групп "
            "для заявок на соревнования. Каждая заявка формирует категорию "
            "«дисциплина + вид программы + пол + возрастная группа».",
            note_style,
        ),
    ]

    story.append(Paragraph("1. Дисциплины", styles["Heading2"]))
    story.append(_make_table(
        [["Код", "Название", "Описание"]] + [[code, label, desc] for code, label, desc in DISCIPLINES],
        header=True,
    ))
    story.append(Spacer(1, 0.35 * cm))

    story.append(Paragraph("2. Ката — виды программы", styles["Heading2"]))
    story.append(Paragraph(
        "При подаче заявки выбирается один из видов программы. Конкретная ката "
        "из реестра ФВКР назначается секретарём на соревновании.",
        note_style,
    ))
    kata_rows = [[f"{i}. {name}"] for i, name in enumerate(KATA_PROGRAM_TYPES, start=1)]
    story.append(_make_table(kata_rows))
    story.append(Spacer(1, 0.35 * cm))

    kumite_sections = [
        ("kumite_ok", "3. Кумитэ ОК (ограниченный контакт) — весовые категории"),
        ("kumite_pk", "4. Кумитэ ПК (полный контакт) — весовые категории"),
        ("kumite_sz", "5. Кумитэ СЗ (средства защиты) — весовые категории"),
    ]
    for discipline, heading in kumite_sections:
        story.append(Paragraph(heading, styles["Heading2"]))
        names = WEIGHT_CATEGORIES.get(discipline, [])
        regular = [n for n in names if n not in NO_WEIGH_CATEGORIES]
        special = [n for n in names if n in NO_WEIGH_CATEGORIES]
        rows = []
        row = []
        for i, name in enumerate(regular, start=1):
            row.append(f"{i}. {_weight_display(name)}")
            if len(row) >= 4:
                rows.append(row)
                row = []
        if row:
        ...
        if special:
            ...
            story.append(Paragraph("Особые категории (без взвешивания):", styles["Heading3"]))
            story.append(_make_table([[f"• {s}" for s in special]]))
        story.append(Spacer(1, 0.35 * cm))

    story.append(Paragraph("6. Возрастные группы", styles["Heading2"]))
    story.append(Paragraph(
        "Определяются автоматически по дате рождения (спортивный возраст: "
        "год турнира минус год рождения). Группа 10–11 лет — только для ката.",
        note_style,
    ))
    age_rows = [["Спортивный возраст", "Мужчины / юноши", "Женщины / девушки", "Примечание"]]
    for g in AGE_GROUPS:
        note = "только ката" if g["kata_only"] else ""
        age_rows.append([g["sport_age"], g["male"], g["female"], note])
    story.append(_make_table(age_rows, header=True))
    story.append(Spacer(1, 0.35 * cm))

    story.append(Paragraph("7. Командные категории", styles["Heading2"]))
    team_rows = [["Категория", "Вид программы", "Состав"]] + [
        [t["title"], t["programs"], t["size"]] for t in TEAM_CATEGORIES
    ]
    story.append(_make_table(team_rows, header=True))
    story.append(Spacer(1, 0.35 * cm))

    story.append(Paragraph("8. Спортивные разряды", styles["Heading2"]))
    rank_rows = []
    row = []
    for i, rank in enumerate(RANKS, start=1):
        ...
        if len(row) >= 3:
            ...
            row = []
    if row:
        rank_rows.append(row + [""] * (3 - len(row)))
    story.append(_make_table(rank_rows))
    story.append(Spacer(1, 0.35 * cm))

    story.append(Paragraph("9. Как формируется категория на турнире", styles["Heading2"]))
    story.append(_make_table([
        ["Элемент", "Пример"],
        ["Дисциплина", "Кумитэ ОК"],
        ["Вид программы / вес", "ОК 70 кг"],
        ["Пол", "М / Ж"],
        ["Возрастная группа", "Юниоры 16-17"],
        ["Итоговая категория", "Кумитэ ОК / 70 кг / М / Юниоры 16-17"],
    ], header=True))
